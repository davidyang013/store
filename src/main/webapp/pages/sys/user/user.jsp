...
<html>
  ...
  <script type="text/javascript">
  	function add(){
		openDlg("<%=basePath%>sys/UserManagerAction!gotoDtl.action?optype=add", 580, 260);
	}
  	...
  	function update(){
  		...
  		openDlg("<%=basePath%>sys/UserManagerAction!gotoDtl.action?id=" + id + "&optype=update", 580, 260);
  	}
  	
  	function query(){
  		openDlg("<%=basePath%>pages/sys/user/userQuery.jsp", 300, 120);
  	}
  	...
  	function queryByCondition(name){
  		...
  		$("#name").val(name);
  		$("#curPage").val("1");
  	    frm.action = "<%=basePath%>sys/UserManagerAction!query.action";
  		frm.submit();
  	}
  	...
  	function del(){
  		...
  		var frm = document.getElementsByTagName("form")[0];
  		frm.action = "<%=basePath%>sys/UserManagerAction!del.action";
  		frm.submit();
  	}
  ...
  <body>
   <form action="" method="post">
	   ...
	   <!-- condition end-->
		...
		<fieldset>
			<legend><strong class="fieldTitle">用户信息管理</strong></legend>	
			...
			<table id="tblMain" class="tblMain" align="center" cellpadding="0" cellspacing="0" onmouseover="lightEffect('tblMain', ${fn:length(resultList) + 1})" >
				...
				<c:forEach begin="${fn:length(resultList) }" end="${user.pageSize - 1 }" step="1" varStatus="state">
				...
				<tr class="trFoot">
					...
					<td align="right" style="padding-right:5px">
						...
						<input type="button" class="btn" value='跳转'
							   onclick="toPage('sys/UserManagerAction!query.action','')">
						<input type="button" class="btn" value='前页'
							   onclick="getLastPage('sys/UserManagerAction!query.action')">
						<input type="button" class="btn" value='后页' 
						       onclick="getNextPage('sys/UserManagerAction!query.action')"> 
					</td>
				</tr>
			</table>
			<!-- menu end -->
